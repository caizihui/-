...
public class Role implements ForRole {
	 ...
	 * 角色的载体，面板
	 */
	static public Battle battle = null;
    ...
    private boolean canGoLeft(int n) {
    	...
    	for(i=1 ; i<=n ; i++) {
    		//有墙不能走
    		if(battle.map.get(this.rolePosition.setY(this.rolePosition.getY()-i)).equals(PictureName.WALL)) {
    			return false;
    		}
    	}
     ...
     */
    public Role(String roleImgName) {
    	this.roleImg = new ImageIcon(roleImgName);
    	this.rolePosition = battle.map.getASpacePoint();
    	this.Mp = 100;
        this.Hp = 30;
        this.Exp = 0;
    	...
    	if(battle != null) {
    		//一队成员进入一队集合
    		if(Function.isFirstRole(this.roleImg.getDescription())) {
    			battle.firstRole.add(this);
    		}
    		
    		else {
    			battle.secondRole.add(this);
    		}
    	...
    	Server.sendedOrder = "增加"+Function.translate(this.roleImg.getDescription())+this.rolePosition.toString();
    	Server.sendedOrderIsChanged = true;
    }
	
    public Role(String roleImgName,Point rolePosition) {
    	this.roleImg = new ImageIcon(roleImgName);
    ...
    public void enterMap() {
		battle.map.replace(this.rolePosition, roleImg.getDescription());
	}

    public void exitMap() {
		battle.map.sharpReplace(this.rolePosition, PictureName.SPACE);
	}
    ...
    public boolean goForward(String direction) {
		if(direction.equals("上")) {
			if(battle.map.get(this.rolePosition.setX(this.rolePosition.getX()-1)).equals(PictureName.SPACE)) { //如果可以往上走
				battle.map.sharpReplace(this.rolePosition, PictureName.SPACE);
    			this.rolePosition = this.rolePosition.setX(this.rolePosition.getX()-1);
    			battle.map.replace(this.rolePosition, this.roleImg.getDescription());
    		...
    		if(battle.map.get(this.rolePosition.setY(this.rolePosition.getY()-1)).equals(PictureName.SPACE)) { //如果可以往左走
    			battle.map.sharpReplace(this.rolePosition, PictureName.SPACE);
    			this.rolePosition = this.rolePosition.setY(this.rolePosition.getY()-1);
    			battle.map.replace(rolePosition, this.roleImg.getDescription());
